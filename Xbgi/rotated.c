    ...
    int bg;
{
    ...
    if((fabs(angle) < 0.001) && (fabs(style.xmagnify - 1.0) < 0.01) &&
       (fabs(style.ymagnify - 1.0) < 0.01))
    ...
    if(align==TLEFT || align==TCENTRE || align==TRIGHT)
        hot_y=(float)item->rows_in/2*style.ymagnify;
    else if(align==MLEFT || align==MCENTRE || align==MRIGHT)
	hot_y=0;
    else if(align==BLEFT || align==BCENTRE || align==BRIGHT)
	hot_y=-(float)item->rows_in/2*style.ymagnify;
    else
	hot_y=-((float)item->rows_in/2-(float)font->descent)*style.ymagnify;
    
    /* x position */
    if(align==TLEFT || align==MLEFT || align==BLEFT || align==NONE)
	hot_x=-(float)item->max_width/2*style.xmagnify;
    ...
    int align;
{
	...
	if(strcmp(text, i1->text)==0 &&
	   ...
	   style.xmagnify==i1->xmagnify &&
	   style.ymagnify==i1->ymagnify &&
	   (i1->nl==1 ||
	    ((align==0)?9:(align-1))%3==
	...
	item->angle=angle;
	item->align=align;
	item->xmagnify=style.xmagnify;
	item->ymagnify=style.ymagnify;
    ...
    int align;
{
	...
	item->corners_x[ic]=((float)xp-(float)item->cols_in/2)*style.xmagnify;
	item->corners_y[ic]=((float)(yp-font->ascent)-(float)item->rows_in/2)
	    *style.ymagnify;
	item->corners_x[ic+1]=item->corners_x[ic];
	item->corners_y[ic+1]=item->corners_y[ic]+(float)height*style.ymagnify;
	item->corners_x[item->nl*4-1-ic]=item->corners_x[ic]+
	    (float)overall.rbearing*style.xmagnify;
    ...
    if(style.xmagnify!=1. && style.ymagnify!=1) {
	I_in=XRotMagnifyImage(dpy, I_in);

	old_cols_in=item->cols_in;
	old_rows_in=item->rows_in;
	item->cols_in=(float)item->cols_in*style.xmagnify;
	item->rows_in=(float)item->rows_in*style.ymagnify;
    }
    ...
    XImage *ximage;
{
    ...
    /* size of final image */
    cols_out=(float)cols_in*style.xmagnify;
    rows_out=(float)rows_in*style.ymagnify;
    ...
    /* for speed */
    xmag_inv=1./style.xmagnify;
    ymag_inv=1./style.ymagnify;
    ...
    int align;
{
    ...
    /* y position */
    if(align==TLEFT || align==TCENTRE || align==TRIGHT)
        hot_y=(float)rows_in/2*style.ymagnify;
    else if(align==MLEFT || align==MCENTRE || align==MRIGHT)
	hot_y=0;
    else if(align==BLEFT || align==BCENTRE || align==BRIGHT)
	hot_y=-(float)rows_in/2*style.ymagnify;
    else
	hot_y=-((float)rows_in/2-(float)font->descent)*style.ymagnify;
    
    /* x position */
    if(align==TLEFT || align==MLEFT || align==BLEFT || align==NONE)
	hot_x=-(float)max_width/2*style.xmagnify;
    else if(align==TCENTRE || align==MCENTRE || align==BCENTRE)
	hot_x=0;
    else
        hot_x=(float)max_width/2*style.xmagnify;
    ...
    xp_in[0].x=-(float)cols_in*style.xmagnify/2-style.bbx_pad;
    xp_in[0].y= (float)rows_in*style.ymagnify/2+style.bbx_pad;
    xp_in[1].x= (float)cols_in*style.xmagnify/2+style.bbx_pad;
    xp_in[1].y= (float)rows_in*style.ymagnify/2+style.bbx_pad;
    xp_in[2].x= (float)cols_in*style.xmagnify/2+style.bbx_pad;
    xp_in[2].y=-(float)rows_in*style.ymagnify/2-style.bbx_pad;
    xp_in[3].x=-(float)cols_in*style.xmagnify/2-style.bbx_pad;
    xp_in[3].y=-(float)rows_in*style.ymagnify/2-style.bbx_pad;
    xp_in[4].x=xp_in[0].x;
    xp_in[4].y=xp_in[0].y;
